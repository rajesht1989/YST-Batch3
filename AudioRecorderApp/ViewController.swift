...
class ViewController: UIViewController,AVAudioPlayerDelegate,AVAudioRecorderDelegate, UITextFieldDelegate {
    ...
    @IBAction func saveBtnTapped(_ sender: Any) {
        ...
        let soundsContext = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
                ...
                let sounds = NSEntityDescription.insertNewObject(forEntityName: "Sounds", into: soundsContext)

               var noteSoundTitle:String = "Sounds"
                
                if (soundTitileTextField.text?.isEmpty)! {
                    noteSoundTitle = "Sounds"
                    soundTitileTextField.text = noteSoundTitle
                } else {
                    noteSoundTitle =  soundTitileTextField.text!
                }
                
//                sounds.noteSoundTitle =  noteSoundTitle
                do {
                    try soundsContext.save()
                } catch _ {
                }
                saveInformationLabel.alpha = 1
                saveInformationLabel.text = "Saved " + noteSoundTitle
